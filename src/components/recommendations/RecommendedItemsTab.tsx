...
export default function RecommendedItemsTab({ items = [] }: RecommendedItemsTabProps) {
  if (!items || items.length === 0) {
    return (
      <div className="text-center py-16 px-4 bg-zinc-50 dark:bg-zinc-800/30 rounded-3xl border border-dashed border-zinc-200 dark:border-zinc-800">
        <Sparkles className="w-10 h-10 text-zinc-400 mx-auto mb-3 opacity-50" />
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between text-lg font-normal text-zinc-500 px-1">
        <span>
          Showing <strong className="font-medium text-zinc-800 dark:text-zinc-200">{items.length}</strong> ranked dish recommendations
        </span>
      ...
      {items.map((item) => {
        ...
        return (
          <div
            ...
            className="p-6 border border-zinc-200 dark:border-zinc-800 rounded-3xl bg-white dark:bg-zinc-900 shadow-sm hover:border-amber-300 dark:hover:border-amber-700/60 transition duration-200 space-y-4"
          >
            {/* Header / Main Details */}
            <div className="flex flex-wrap items-start justify-between gap-4">
              <div className="flex items-start gap-4 min-w-0">
                {/* Rank Badge */}
                <div className="flex-shrink-0 flex items-center justify-center w-11 h-11 rounded-2xl bg-amber-100 dark:bg-amber-950/70 text-amber-800 dark:text-amber-300 font-medium text-lg shadow-xs border border-amber-200 dark:border-amber-800/60">
                  #{item.rankPosition}
                </div>

                <div className="min-w-0 space-y-1">
                  <div className="flex flex-wrap items-center gap-2.5">
                    <h4 className="font-medium text-2xl text-zinc-800 dark:text-zinc-100 truncate">
                      {item.menuItemName || `Menu Item #${item.menuItemId}`}
                    </h4>
                    {item.isExploration && (
                      <span className="inline-flex items-center gap-1.5 text-lg font-normal px-3 py-0.5 rounded-full bg-indigo-50 text-indigo-700 dark:bg-indigo-950/60 dark:text-indigo-300 border border-indigo-200 dark:border-indigo-800/60">
                        <Compass className="w-4 h-4" /> Exploration
                      </span>
                    )}
                    {item.candidateSource && (
                      <span className="text-lg font-mono font-normal px-2.5 py-0.5 rounded-full bg-zinc-100 dark:bg-zinc-800 text-zinc-500">
                        {item.candidateSource}
                      </span>
                    )}
                  </div>

                  <div className="flex flex-wrap items-center gap-x-5 gap-y-1.5 text-lg font-normal text-zinc-500 dark:text-zinc-400">
              ...
              <div className="flex flex-col items-end flex-shrink-0">
                <span
                  className={`inline-flex items-center gap-2 text-lg font-normal px-4 py-1.5 rounded-full shadow-xs ${
                    isHighMatch
                ...
                {item.groupScore !== undefined && item.groupScore !== null && (
                  <span className="text-lg font-normal text-purple-700 dark:text-purple-400 mt-1">
            ...
            {/* AI Reasoning Text */}
            {item.reasonText && (
              <div className="text-lg font-normal text-amber-900 dark:text-amber-200 bg-amber-50/90 dark:bg-amber-950/40 p-4 rounded-2xl border border-amber-200/80 dark:border-amber-800/40 flex items-start gap-3">
                <span className="text-xl">💡</span>
                <div className="flex-1 space-y-1.5">
                  <p className="leading-relaxed font-normal">{item.reasonText}</p>
                  {item.reasonCodes && item.reasonCodes.length > 0 && (
                    <div className="flex flex-wrap gap-2 mt-2">
                      {item.reasonCodes.map((code) => (
                        <span
                          key={code}
                          className="inline-flex items-center gap-1 text-lg font-mono font-normal px-2.5 py-0.5 bg-amber-100 dark:bg-amber-900/60 text-amber-800 dark:text-amber-300 rounded-full"
                        >
            ...
            {item.scoreBreakdown && Object.keys(item.scoreBreakdown).length > 0 && (
              <div className="pt-3 border-t border-zinc-100 dark:border-zinc-800 space-y-2.5">
                <div className="flex items-center justify-between text-lg">
                  <span className="font-normal text-zinc-500 uppercase tracking-wider">
                    Score Strategy Breakdown
                  </span>
                  <span className="text-zinc-400 font-mono text-lg font-normal">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                  {Object.entries(item.scoreBreakdown).map(([strategy, score]) => {
                    ...
                    return (
                      <div
                        key={strategy}
                        className="bg-zinc-50 dark:bg-zinc-800/60 p-3.5 rounded-2xl border border-zinc-200/70 dark:border-zinc-800 space-y-2"
                      >
