import { Pencil, RotateCcw, Salad, Trash2 } from "lucide-react";
...
type Props = {
  items: DietaryType[];
  disabled?: boolean;
  onEdit: (item: DietaryType) => void;
  onDelete: (item: DietaryType) => void;
  onRestore: (item: DietaryType) => void;
};
      ...
      <table className="w-full min-w-[1050px] border-collapse">
        <thead>
          <tr className="border-b border-gray-100 bg-[#fbfcfb] text-left">
            <th className="px-5 py-4 text-lg font-bold text-[#136C34]">របបអាហារ</th>
            <th className="px-5 py-4 text-lg font-bold text-[#136C34]">កូដ</th>
            <th className="px-5 py-4 text-lg font-bold text-[#136C34]">ប្រភេទ</th>
        ...
        <tbody>
          {items.map((item) => (
            <tr key={item.uuid} className="border-b border-gray-100 last:border-0 hover:bg-gray-50/60">
              <td className="px-5 py-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="mt-0.5 max-w-[200px] truncate text-xs text-gray-400">{item.uuid}</p>
                  </div>
                </div>
              </td>
              ...
              <td className="max-w-[320px] px-5 py-4 text-sm leading-6 text-gray-500">
