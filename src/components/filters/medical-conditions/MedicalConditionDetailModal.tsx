  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const json = (await res.json()) as ApiResponseEnvelope<MedicalCondition> | MedicalCondition;
        if (!isMounted) return;
        let detail: MedicalCondition | null = null;
        if (json && typeof json === "object") {
          if ("payload" in json && json.payload) detail = json.payload as MedicalCondition;
          else if ("data" in json && json.data) detail = json.data as MedicalCondition;
          else detail = json as MedicalCondition;
        }
        setData(detail || item);
      } catch (err) {
        if (!isMounted) return;
        console.warn(`[MedicalConditionDetailModal]`, err);
        setIsError(true);
        setData(item);
      } finally {
          ...
          <div className="flex min-w-0 items-center gap-4">
            ...
            <div className="min-w-0">
              <p className="truncate text-3xl font-semibold text-primary-800">
                {displayItem.name || "ព័ត៌មានស្ថានភាពសុខភាព"}
              </p>
              <p className="mt-1 text-lg text-gray-500">ស្ថានភាពសុខភាព (Medical Conditions)</p>
            </div>
          </div>
        ...
        <div className="space-y-6 p-6 sm:p-8">
          {isLoading && !data ? (
            ...
            <div className="rounded-2xl border border-red-100 bg-red-50 p-5 text-lg font-medium text-red-600">
              មិនអាចទាញយកព័ត៌មានលម្អិតស្ថានភាពសុខភាពបានទេ។
            </div>
          ) : (
