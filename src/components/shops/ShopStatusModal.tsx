...
export default function ShopStatusModal({
  ...
  const [review, { isLoading: rLoading }] =
    useUpdateStoreReviewStatusMutation();
  const [accountUpdate, { isLoading: aLoading }] =
    useUpdateStoreAccountStatusMutation();
  const [operatingUpdate, { isLoading: oLoading }] =
    useUpdateStoreOperatingStatusMutation();
        ...
        <div className="flex items-start justify-between border-b px-6 py-5">
          <div>
            <h2 className="flex items-center gap-2 text-xl font-black">
              <Settings2 size={20} className="text-[#137A3D]" />
              Manage Store status
            </h2>
            <p className="mt-1 text-sm text-gray-500">{store.storeName}</p>
          </div>
          <button
            disabled={loading}
            onClick={onClose}
        ...
        <div className="p-6">
            ...
            {(["REVIEW", "ACCOUNT", "OPERATING"] as StoreStatusAction[]).map(
              (x) => (
                <button
                  key={x}
                  onClick={() => setAction(x)}
                  className={`rounded-xl px-3 py-2.5 text-xs font-black ${action === x ? "bg-white text-[#137A3D] shadow-sm" : "text-gray-500"}`}
                >
                  {x}
                </button>
              ),
          ...
          <div className="mt-5">
            {action === "REVIEW" && (
              <div className="space-y-4">
                ...
                <p className="text-xs text-amber-700">
                  Backend ជាអ្នក enforce valid review transition។
                </p>
              </div>
            )}
            {action === "ACCOUNT" && (
            ...
            {action === "OPERATING" && (
              <select
                ...
                className="h-12 w-full rounded-2xl border px-4"
              >
                {["OPEN", "CLOSED", "TEMPORARILY_CLOSED", "UNKNOWN"].map(
                  (x) => (
                    <option key={x}>{x}</option>
                  ),
                )}
          ...
          <div className="mt-6 flex justify-end gap-3 border-t pt-5">
            ...
            <button
              disabled={loading}
              onClick={() => void save()}
              className="inline-flex items-center gap-2 rounded-xl bg-[#137A3D] px-4 py-2.5 font-black text-white disabled:opacity-60"
            >
