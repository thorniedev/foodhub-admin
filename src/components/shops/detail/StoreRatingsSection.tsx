...
export default function StoreRatingsSection({ store }: { store: Store }) {
  return (
    <Section title="Ratings & review state" icon={<Star size={20} />}>
      <div className="grid gap-3 sm:grid-cols-2">
        <Card label="Average rating" value={formatRating(store.averageRating)} />
        <Card label="Total reviews" value={String(store.totalReviews ?? 0)} />
        <Card label="Hygiene rating" value={formatRating(store.hygieneRating)} />
...
function Card({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-2xl bg-emerald-50/70 px-4 py-4">
      ...
      <p className="mt-1 text-xl text-[#137A3D]">{value}</p>
    </div>
  );
}
