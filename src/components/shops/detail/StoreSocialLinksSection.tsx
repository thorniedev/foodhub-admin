...
export default function StoreSocialLinksSection({
  ...
  return (
    <Section
      title={`Social links (${links.length})`}
      icon={<Share2 size={18} />}
    >
      {links.length === 0 ? (
          ...
          {[...links]
            ...
            .map((l, i) => (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="flex items-center justify-between rounded-2xl bg-gray-50 px-4 py-3 hover:bg-emerald-50"
              >
                <div className="min-w-0">
                  <p className="font-black">{l.platform}</p>
                  <p className="truncate text-xs text-gray-400">
                    {l.profileUrl}
                  </p>
                </div>
                <ExternalLink size={17} className="text-[#137A3D]" />
              </a>
            ))}
        </div>
      )}
