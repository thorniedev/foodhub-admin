...
export default function StoreHoursModal({
  ...
  useEffect(() => {
    if (open && data) {
      setHours(data.map((x) => ({ ...x })));
      setError(null);
    }
  }, [open, data]);
  if (!open) return null;
  const patch = (i: number, p: Partial<StoreHour>) =>
    setHours((c) => c.map((x, n) => (n === i ? { ...x, ...p } : x)));
  const save = async () => {
    for (const [i, h] of hours.entries()) {
      if (
        h.scheduleType === "WEEKLY" &&
        (!h.dayOfWeek || h.dayOfWeek < 1 || h.dayOfWeek > 7)
      )
        return setError(`Row ${i + 1}: dayOfWeek 1..7`);
      if (h.scheduleType === "SPECIAL_DATE" && !h.businessDate)
        return setError(`Row ${i + 1}: businessDate required`);
      if (!h.isClosed && (!h.openingTime || !h.closingTime))
        return setError(`Row ${i + 1}: opening/closing required`);
    }
    try {
      await replace({
        storeUuid,
        body: {
          hours: hours.map((h) => ({
            ...h,
            openingTime: h.isClosed ? null : h.openingTime,
            closingTime: h.isClosed ? null : h.closingTime,
            dayOfWeek: h.scheduleType === "WEEKLY" ? h.dayOfWeek : null,
            businessDate:
              h.scheduleType === "SPECIAL_DATE" ? h.businessDate : null,
          })),
        },
      }).unwrap();
      await refetch();
      ...
      <div className="max-h-[94vh] w-full max-w-6xl overflow-y-auto rounded-[30px] bg-white shadow-2xl">
        <div className="sticky top-0 z-10 flex justify-between border-b bg-white px-6 py-5">
          <div>
            <h2 className="flex items-center gap-2 text-xl font-black">
              <Clock3 className="text-[#137A3D]" />
              Store opening hours
            </h2>
            <p className="text-sm text-gray-500">
              GET + PUT /admin/stores/{storeUuid}/hours
            </p>
          </div>
          <button
            disabled={saving}
        ...
        <div className="p-6">
          <div className="rounded-2xl bg-amber-50 px-4 py-3 text-xs leading-5 text-amber-800">
            <b>Replace operation:</b> load full list, edit it, then PUT full
            `hours`. Overnight (18:00 → 02:00) is allowed.
          </div>
          {isLoading ? (
            <div className="p-16 text-center">
              <Loader2 className="mx-auto animate-spin text-[#137A3D]" />
            </div>
          ) : loadError ? (
            <div className="mt-5 bg-red-50 p-4 text-red-600">
              ...
              <div className="mt-5 space-y-3">
                {hours.length === 0 ? (
                  ...
                  hours.map((h, i) => (
                    <div
                      key={i}
                      className="grid gap-3 rounded-2xl bg-gray-50 p-4 xl:grid-cols-[150px_130px_150px_150px_100px_110px_1fr_44px]"
                    >
                      <select
                        value={h.scheduleType}
                        onChange={(e) =>
                          patch(
                            i,
                      ...
                      >
                        ...
                        <option>SPECIAL_DATE</option>
                      </select>
                      {h.scheduleType === "WEEKLY" ? (
                        <select
                          value={h.dayOfWeek ?? 1}
                          onChange={(e) =>
                            patch(i, { dayOfWeek: Number(e.target.value) })
                          }
                          className="h-11 rounded-xl border bg-white px-3"
                        >
                          {[1, 2, 3, 4, 5, 6, 7].map((d) => (
                        ...
                        <input
                          type="date"
                          value={h.businessDate ?? ""}
                          onChange={(e) =>
                            patch(i, { businessDate: e.target.value })
                          }
                          className="h-11 rounded-xl border bg-white px-3"
                        />
                      )}
                      <input
                        ...
                        className="h-11 rounded-xl border bg-white px-3 disabled:opacity-40"
                      />
                      <input
                        type="time"
                        step="1"
                        disabled={h.isClosed}
                        value={h.closingTime?.slice(0, 8) ?? ""}
                        onChange={(e) =>
                          patch(i, {
                            closingTime:
                              e.target.value.length === 5
                      ...
                      <input
                        type="number"
                        min="1"
                        value={h.intervalOrder}
                        onChange={(e) =>
                          patch(i, {
                            intervalOrder: Number(e.target.value) || 1,
                          })
                        }
                        className="h-11 rounded-xl border bg-white px-3"
                      />
                      <label className="flex h-11 items-center gap-2 rounded-xl border bg-white px-3 text-xs">
                        <input
                          type="checkbox"
                          checked={h.isClosed}
                          onChange={(e) =>
                            patch(i, { isClosed: e.target.checked })
                          }
                        />
                        Closed
                      </label>
                      <input
                        value={h.reason ?? ""}
                        onChange={(e) =>
                          patch(i, { reason: e.target.value || null })
                        }
                        placeholder="Reason"
                        className="h-11 rounded-xl border bg-white px-3"
                      />
          ...
          <div className="mt-6 flex justify-end gap-3 border-t pt-5">
            ...
            <button
              ...
              onClick={() => void save()}
              className="inline-flex items-center gap-2 rounded-xl bg-[#137A3D] px-4 py-2.5 font-black text-white disabled:opacity-50"
            >
              {saving ? (
