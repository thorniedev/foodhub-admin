...
export default function DynamicContentPreview({
  ...
  return (
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-3 sm:p-4">
      <div className="bg-white rounded-2xl w-full max-w-xs p-4 sm:p-5 max-h-[85vh] overflow-y-auto">
        <div className="flex items-center justify-between mb-4">
          <h3 className="font-semibold text-gray-800 text-sm">ការមើលជាមុននៃកម្មវិធីអតិថិជន</h3>
        ...
        {groups.map((group) => {
          ...
          return (
            <div key={group.key} className="mb-4">
              ...
              <div className="flex flex-wrap gap-1.5">
                {items.map((item) => (
                  <span key={item.id} className="px-2.5 py-1 text-xs rounded-lg bg-gray-100 text-gray-600">
                    {item.label}
                  </span>
                ))}
              </div>
