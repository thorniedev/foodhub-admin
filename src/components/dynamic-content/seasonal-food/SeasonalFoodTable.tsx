...
interface SeasonalFoodTableProps {
  ...
  onToggleStatus: (item: SeasonalFoodImage) => void;
}

const STATUS_BADGE: Record<SeasonalFoodImage["status"], string> = {
  active: "bg-emerald-50 text-emerald-600",
  pending: "bg-amber-50 text-amber-600",
  disabled: "bg-gray-100 text-gray-400",
};

const STATUS_LABEL: Record<SeasonalFoodImage["status"], string> = {
  active: "កំពុងបង្ហាញ",
  pending: "កំពុងរង់ចាំ",
  disabled: "បានបិទ",
};
    ...
    <div className="bg-white border border-gray-100 rounded-xl overflow-x-auto">
      <table className="w-full text-sm min-w-[700px]">
        ...
        <tbody>
          ...
          {data.length === 0 && (
            <tr>
              <td colSpan={5} className="py-10 text-center text-[#F97316]">
                មិនមានទិន្នន័យ
              </td>
            </tr>
          )}
