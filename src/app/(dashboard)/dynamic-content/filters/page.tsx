...
//           className="flex items-center justify-center gap-2 text-sm sm:text-base font-medium text-[#136C34] hover:text-emerald-700 px-3 py-2 rounded-lg hover:bg-emerald-50 shrink-0 w-full lg:w-auto"
//         >
//           <Eye size={16} />
//           មើលការបង្ហាញជាមុន
//         </button>
...
export default function DynamicContentPage() {
  ...
  const groupItems = useMemo(() => {
    ...
    if (useAlphaSort) {
      items = [...items].sort((a, b) =>
        sortDirection === "asc" ? a.label.localeCompare(b.label) : b.label.localeCompare(a.label)
      );
    } else {
  ...
  const nextOrder = groupOnlyItems.length > 0 ? Math.max(...groupOnlyItems.map((o) => o.order)) + 1 : 1;
